...
Initiating a payment followed by a capture, and a flow where initiate payment is split into two parts: Prepare-Ready and then followed by a capture.
The recommendation is to use the Initiate-Capture flow if there is no need for knowing the customer before setting the amount.
Using the Initiate-Capture flow will save a call to the API and in the end improve the overall time it takes to complete a payment.
